    ...
    rawEntries.forEach(entry => {
      ...
      if (!grouped[entryId]) {
        grouped[entryId] = {
          ...
          castingItems: [],
          lotFilingMapper: [],
          filingItems: entry.filingItems || [],  // to make sure product/scrap data doesn't vanish
        };
      }

      grouped[entryId].castingItems.push({
        ...
        type: entry.casting_item_type
      });

      grouped[entryId].lotFilingMapper.push(...(entry.lotFilingMapper || []));
    });

    const finalGrouped = Object.values(grouped);
    setFilingEntries(finalGrouped);
    console.log("Grouped after refresh:", finalGrouped);
  } catch (error) {
