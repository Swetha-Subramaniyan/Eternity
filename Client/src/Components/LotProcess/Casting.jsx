...
export default function Casting() {
  ...
  const [itemsList, setItemsList] = useState([]); // for dropdown options
  ...
  <div className="casting-popup-overlay">
    <div className="casting-popup">
      <div className="casting-popup-header">
        Casting / Melting
        <button onClick={() => { setShowPopup(false); resetForm(); }} className="close-btnn">X</button>
      </div>

      <div className="casting-popup-content">
        {/* Column 1 */}
        <div className="casting-popup-column">
          <label>Date</label>
              ...
              <tbody>
                {items.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td><input value={item.remarks} onChange={(e) => handleItemChange(items, setItems, index, "remarks", e.target.value)} /></td>
                    <td>
              ...
              <tbody>
                {scrapItems.map((item, index) => (
                 ...
                 <td><input value={item.name} onChange={(e) => handleItemChange(scrapItems, setScrapItems, index, "name", e.target.value)} /></td>
                    ...
                    <td>{((parseFloat(item.weight || 0) * parseFloat(item.touch || 0)) / 100).toFixed(3)}</td>
                    <td><input value={item.remarks} onChange={(e) => handleItemChange(scrapItems, setScrapItems, index, "remarks", e.target.value)} /></td>
                    <td>
