...
useEffect(() => {
  ...
  const handleSaveViewEntry = async () => {
    ...
    // 🔁 Loop through product items to post them
    for (const product of viewEntry.productItems || []) {
      const data = {
        filing_person_id: 1, // Replace with actual user ID
        casting_item_id: viewEntry.items?.[0]?.id, // assuming first assigned item's ID is used
        type: 'Items',
        item: product.itemName,
        weight: parseFloat(product.weight),
        touch: parseFloat(product.touch),
        item_purity: parseFloat(product.purity),
        remarks: product.remarks,
        after_weight: parseFloat(viewEntry.afterWeight),
        stone_option: product.hasStone === 'Yes' ? 'WithStone' : 'WithoutStone'
      };
  
      try {
        await axios.post(`${BACKEND_SERVER_URL}/api/filingentry`, data);
        console.log("Posted:", data);
      } catch (error) {
        console.error("Failed to post filing entry:", error);
      }
    }
  
    // 🔁 Loop through scrap items and post them separately
    for (const scrap of viewEntry.scrapItems || []) {
      const data = {
        filing_person_id: 1, // Replace with actual user ID
        casting_item_id: viewEntry.items?.[0]?.id,
        type: 'ScrapItems',
        item: scrap.itemName,
        weight: parseFloat(scrap.weight),
        touch: parseFloat(scrap.touch),
        item_purity: parseFloat(scrap.purity),
        remarks: scrap.remarks,
        after_weight: 0,
        stone_option: 'WithoutStone' // Generally scrap won't have stone
      };
  
      try {
        await axios.post(`${BACKEND_SERVER_URL}/api/filingentry`, data);
        console.log("Scrap Posted:", data);
      } catch (error) {
        console.error("Failed to post scrap filing entry:", error);
      }
    }
  
    alert("Filing entry saved and submitted to backend.");
  };
